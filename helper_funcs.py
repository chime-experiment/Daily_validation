...
def plotSens(rev, LSD, vmin=0.995, vmax=1.005):
    ...
    sensrat_mask, times, _ = infill_gaps(sensrat_mask, sens.time, sens.freq)
...
def infill_gaps(data, x, y, xspan=None, yspan=None):
    ...
    def _interp(x, span):
        ...
        dxm = np.median(np.abs(dx))

        # Make a constant grid
        xp = np.arange(span[0], span[1], dxm)
        # Map the existing data values to their indices
        # in the new grid
        map_ = abs(xp[:, np.newaxis] - x[np.newaxis]).argmin(axis=0)
